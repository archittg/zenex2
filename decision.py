...
if __name__ == "__main__":
    # My own test detections — types chosen to exist in both
    # priority.py's default table and mission_modes.py's tables
    my_detections = [
        {"type": "unknown_ship", "confidence": 0.97},   # should be CRITICAL -> TRANSMIT
        {"type": "oil_spill", "confidence": 0.85},       # HIGH -> TRANSMIT
        {"type": "fishing_boat", "confidence": 0.92},    # MEDIUM -> TRANSMIT_COMPRESSED
        {"type": "ship", "confidence": 0.70},            # MEDIUM -> TRANSMIT_COMPRESSED
        {"type": "small_boat", "confidence": 0.50},      # LOW -> DISCARD
        {"type": "cloud", "confidence": 0.90},           # DISCARD -> DISCARD
    ]
